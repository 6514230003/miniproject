...
@if($savedJob > 0)
    <button class="btn btn-block btn-success btn-md" disabled>บันทึกข้อมูลผู้สมัคร</button>
@else
...
<input name="company" type="hidden" value="{{$job->company}}">
@if ($appliedJob > 0)
<button class="btn btn-block btn-primary btn-md" disabled>Success, You've already applied for the job</button>

@else
<button type="submit" name="submit" class="btn btn-block btn-primary btn-md">Apply For This Job</button>
...
<h2 class="section-title mb-2">{{$relatedJobsCount}} ตำแหน่งงานที่เกี่ยวข้อง </h2>
          </div>
        </div>
